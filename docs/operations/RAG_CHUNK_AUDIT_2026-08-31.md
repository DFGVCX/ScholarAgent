...
表格/算法专项审计共发现 20 个表格对象和 9 个算法对象。PyMuPDF 回退路径中 19 个表格只有标题与截图、没有可靠单元格，继续标记 `review/table_cells_unavailable`，不从截图文字伪造表格；这是后续 Docling TableFormer 主路径的明确比较项。算法正文实际存在，但步骤常被压成代码围栏内的一行。切片器现在去掉围栏噪声，识别 `1:`、`1.` 和保守的裸数字动作步骤，补全 `Algorithm N. 标题` 并逐行保留步骤。真实 IEEE 论文的 6 个算法由每个 3 行恢复为 8–14 行，最大 227 token；FLchain 算法恢复为标题加 3 个独立步骤。

对语料目录现存 54 个 `fig/figure` PNG 做像素审计：没有空白文件，有效内容 bbox 占整图比例均不低于 55%，任一侧留白均不超过 20%，当前自动裁剪未复现大面积空白。当前解析统计只有 41 个 figure Chunk，额外文件来自历史解析候选或旧版检测结果；不能在解析开始时直接删除目录，否则可能破坏旧内容版本的资产引用。后续应将截图资产绑定到 content version，成功换版后按明确保留策略清理孤立文件。
