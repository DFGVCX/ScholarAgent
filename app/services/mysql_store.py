...
def _seed_desktop_account() -> None:
    """Create the private desktop tenant in the PostgreSQL runtime."""
    execute(
        "INSERT INTO scholar_tenants (tenant_id, name, metadata_json) VALUES (?, ?, ?) "
        "ON CONFLICT (tenant_id) DO UPDATE SET name=EXCLUDED.name, metadata_json=EXCLUDED.metadata_json",
        ("tenant_local", "ScholarAgent Local", encode_json({"plan": "desktop"})),
    )
    existing = fetch_one(
    ...
    if existing is None:
        execute(
            ...
            (
                ...
                "Local Researcher",
                encode_json(["tenant_admin", "researcher"]),
