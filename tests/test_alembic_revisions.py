...
class AlembicRevisionGraphTest(unittest.TestCase):
    ...
    def test_task_node_runs_follow_pdf_content_generation(self) -> None:
        migration = Path(
            "alembic/versions/20260905_0011_task_node_runs.py"
        ).read_text(encoding="utf-8")

        self.assertIn('down_revision = "20260901_0010"', migration)
        self.assertIn("CREATE TABLE IF NOT EXISTS scholar_task_node_runs", migration)
        self.assertIn("idx_task_node_runs_fingerprint", migration)
